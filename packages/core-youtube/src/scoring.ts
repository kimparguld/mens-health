...
// Universal clickbait tropes — apply regardless of topic vertical.
...
/**
 * `extraPatterns` lets a site add clickbait phrasing specific to its own
 * topic vertical (e.g. "5x testosterone" for a men's-health site) on top of
 * the universal set above.
 */
export function detectsClickbait(
  title: string,
  extraPatterns: RegExp[] = [],
): boolean {
  return [...CLICKBAIT_PATTERNS, ...extraPatterns].some((pattern) =>
    pattern.test(title),
  );
}
...
export function scoreVideo(
  input: ScoreInput,
  title: string,
  extraClickbaitPatterns: RegExp[] = [],
): ScoreOutput {
  const isClickbait = detectsClickbait(title, extraClickbaitPatterns);
