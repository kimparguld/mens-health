...
/**
 * Binds the client to this site's own YouTube Data API key once, so callers
 * keep calling `searchVideos(...)`/`fetchVideoDetails(...)`/
 * `searchAndEnrichVideos(...)` exactly as before (see
 * apps/menhealth/lib/youtube/client.ts).
 */
export function createYouTubeClient(apiKey: string) {
  ...
  async function searchAndEnrichVideos(
    query: string,
    maxResults = 25,
  ...
  return { searchVideos, fetchVideoDetails, searchAndEnrichVideos };
}
