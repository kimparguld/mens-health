...
function makePost(overrides: PartialPost = {}) {
  return {
    ...
    hashtags: ["MensHealth"],
    utmUrl:
      "https://hype-check.net/videos/sleep-testosterone?utm_source=x",
    riskLevel: "LOW",
...
describe("XAdapter.validate()", () => {
  ...
  const shortPost = makePost({
    ...
    hashtags: ["MensHealth"],
    utmUrl: "https://hype-check.net/videos/s?utm_source=x",
  });
  ...
  it("rejects when caption + UTM URL exceeds 280 chars", async () => {
    ...
    const result = await adapter.validate(
      makePost({
        ...
        caption: longCaption,
        hashtags: [],
        utmUrl: "https://hype-check.net/videos/x?utm_source=x",
      }),
    );
  ...
  it("does not double-append UTM URL if already in caption", async () => {
    ...
    const utmUrl = "https://hype-check.net/videos/s?utm_source=x";
