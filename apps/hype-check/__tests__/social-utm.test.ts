...
describe("buildUtmUrl", () => {
  it("produces the correct UTM parameters for YouTube Community", () => {
    ...
    expect(url).toBe(
      "https://www.hype-check.net/videos/my-video?utm_source=youtube&utm_medium=community&utm_campaign=claim_check",
    );
  });
  ...
  it("uses a custom baseUrl when supplied", () => {
    const url = buildUtmUrl({
      ...
      path: "/topics/fitness",
      campaign: "topic",
      baseUrl: "https://staging.hype-check.net",
    });
    expect(url).toContain("staging.hype-check.net");
