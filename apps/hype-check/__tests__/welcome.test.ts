...
const APP_URL = "https://hype-check.com";
const UNSUB_URL = "https://hype-check.com/api/newsletter/unsubscribe?id=sub_123";

describe("welcomeSubject", () => {
  it("mentions the site name", () => {
    expect(welcomeSubject).toContain("Hype Check");
  });
});
...
describe("buildWelcomeHtml", () => {
  ...
  it("includes the disclaimer", () => {
    ...
    expect(html).toContain("does not constitute financial, legal, or investment advice");
  });
});
...
describe("buildWelcomeText", () => {
  ...
  it("includes the disclaimer", () => {
    const text = buildWelcomeText(APP_URL, UNSUB_URL);
    expect(text).toContain("does not constitute financial, legal, or investment advice");
  });
});
