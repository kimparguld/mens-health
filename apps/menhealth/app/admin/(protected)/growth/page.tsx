...
const sections = [
  {
    ...
    links: [
      ...
      {
        label: 'Ad video packages',
        href: '/admin/social/video-ads',
        desc: 'Generate and approve downloadable ad video bundles',
      },
      {
            ...
            <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {section.links.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="flex flex-col rounded-xl border border-gray-200 bg-white px-4 py-4 transition-all hover:border-emerald-300 hover:shadow-sm"
                  >
