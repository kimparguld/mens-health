...
export function BulkConfirmMediumButton() {
  ...
  async function runBulkConfirm() {
    if (
      !window.confirm(
        "This applies the AI-suggested verdict to every pending MEDIUM-risk claim with no per-claim review. Continue?",
      )
    ) {
      return;
    }
  ...
  return (
    <div className="mb-5 flex flex-col gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <div className="flex items-center justify-between gap-4">
        <p>
          Bulk-confirm all pending MEDIUM-risk claims: accepts the
          AI-suggested verdict for every MEDIUM-risk claim awaiting your
          confirm, with no per-claim review. HIGH-risk claims are never
        ...
        >
          {running ? "Confirming…" : "Bulk-confirm MEDIUM risk"}
        </button>
      </div>

      {confirmed !== null && (
        <p className="text-xs text-amber-800">
          Confirmed <strong>{confirmed}</strong> MEDIUM-risk claim
          {confirmed !== 1 ? "s" : ""}.
        </p>
      )}
