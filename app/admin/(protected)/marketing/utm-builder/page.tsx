...
export default function UtmBuilderPage() {
  const [fields, setFields] = useState<UtmField>({
    ...
    content: "",
    term: "",
    destinationUrl: "https://menhealth-digest.com/newsletter",
  });
        ...
        <div className="flex flex-wrap gap-2">
          {PRESET_SOURCES.map((p) => (
            <button
              ...
              className="rounded-full border border-gray-200 px-3 py-1 text-xs text-gray-600 transition-colors hover:border-emerald-400 hover:text-emerald-800"
            >
              {p.label}
            </button>
          ))}
          ...
          [
            {
              ...
              placeholder: "https://menhealth-digest.com/newsletter",
            },
            {
              key: "source",
              label: "utm_source",
      ...
      {utmUrl && (
        <div className="mt-8">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
            <code className="min-w-0 flex-1 text-xs break-all text-emerald-900">
              {utmUrl}
            </code>
            <button
              onClick={copyUrl}
              className="flex-shrink-0 rounded bg-emerald-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-800"
            >
              {copied ? "Copied!" : "Copy"}
            </button>
          </div>
